...
-- Locations
CREATE TABLE IF NOT EXISTS country (country_name VARCHAR PRIMARY KEY);
...
FROM
    airport
    JOIN city ON city.country_name = airport.country_name
    AND city.city_name = airport.city_name;
...
CREATE TABLE IF NOT EXISTS purchase (
    ...
    offer_price INTEGER,
    offer_class VARCHAR -- TODO: transactions
);
...
SELECT
    ...
    flight.departure_utc :: TIMESTAMP WITH TIME ZONE AT TIME ZONE (
        ...
        FROM
            airport_timezone
        WHERE
            airport_timezone.iata_code = flight.origin
    ) AS departure_local_time,
    (flight.departure_utc + flight.duration) :: TIMESTAMP WITH TIME ZONE AT TIME ZONE (
        ...
        FROM
            airport_timezone
        WHERE
            airport_timezone.iata_code = flight.destination
    ) AS arrival_local_time,
    ...
    (
        (
            ...
            FROM
                aircraft_view
            WHERE
                aircraft_view.registration = flight.aircraft
        ) - (
            SELECT
                COUNT(*)
            FROM
                purchase
            WHERE
                purchase.flight_serial = flight.flight_serial
                AND offer_class = 'Y'
        )
    ) AS y_class_free_capacity,
    (
        (
            SELECT
                j_class_capacity
